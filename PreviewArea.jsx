 ...
 */
  ...
  useEffect(() => {
    ...
    if (parseInt(timer) > 0) {
      timeoutId = setTimeout(() => {
        console.log("animation started after delay");
        setIsAnimating(true);
      }, parseInt(timer) * 1000);
    }
  ...
  const animateStep = useCallback(() => {
    ...
    setSprites((prevSprites) => {
      ...
      for (let key in currentMoveData) {
        let mini = currentDirection[String(key)]["min"];
        if (mini < currentDirection[String(key)]["max"]) {
          const {
            vx = 0,
            vy = 0,
            vr = 0,
            ...
            time,
            action,
          } = currentMoveData[String(key)][mini];
          const spriteIndex = nextSprites.findIndex(
            (s) => s.id === parseInt(key)
          );
          if (spriteIndex === -1) continue;
          const sprite = nextSprites[spriteIndex];
          const dir = localDir[String(sprite.id)]["dirn"] ?? 1;
          ...
          for (let i = 0; i < nextSprites.length; i++) {
            ...
            if (side) {
              ...
              pY = sprite.y;
              localDir[String(sprite.id)]["dirn"] *= -1;
              localDir[String(other.id)]["dirn"] *= -1;
              break;
            }
          }
          console.log(pX, pY);
          ...
          if (action === "REP") {
            ...
            inc = currentDirection[String(key)]["min"] += 1;
          }
          ...
          setCurrentDirection((prev) => ({
            ...
            [String(key)]: {
              ...prev[String(key)],
              min: inc,
              dirn: localDir[String(sprite.id)]["dirn"],
            },
          }));
        }
      }
